...
// InsertAttackSessions inserts one or more AttackSessionLog entries into the DB
func InsertAttackSessions(ctx context.Context, db *sql.DB, sessions []AttackSessionLog) error {
	if len(sessions) == 0 {
		return nil
	}

...
func RunServerMode(simConfig config_manager.SimulationConfig, trackingConfig config_manager.TrackingConfig) {
	settings := engine.NewMTPMSettings([]int{3}, []int{3}, 3, 1, 1, "HEBBIAN", "NO_OVERLAP")
	trackedState := engine.NewTrackedSession(settings, 1_000_000)
